...
export default function CRMDashboard() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const data = await response.json()
      if (data.success) {
        setLeads(data.leads.slice(0, 10)) // Show latest 10 leads
      }
    } catch (error) {
  ...
  const fetchDeals = async () => {
    try {
      ...
      const data = await response.json()
      if (data.success) {
        setDeals(data.deals.slice(0, 10)) // Show latest 10 deals
      }
    } catch (error) {
          ...
          {activeTab === 'leads' && (
            <div className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_#000] p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button className="bg-red-600 text-white border-4 border-black font-bold px-4 py-2 shadow-[4px_4px_0px_0px_#000] hover:shadow-[6px_6px_0px_0px_#000] transition-all duration-200 flex items-center space-x-2">
